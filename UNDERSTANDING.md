...
Now we have concept of Apps, what IAm supports is an identity and authentication layer for all these apps and that every app can communicate and so these apps don't have to manage the authentication layer themselves. These apps can be created, for each applications it's gonna need: `app_id, webhook_url, webhook_secret (given by the user for the webhook), app_secret (autogenerated), webhook_scope (scope of webhook), base_url (the url of the application backend), org_id (carbon must be org_owner or admin), app_scope (this would also include external apps that the app relies on for obo)` there are also some optional parameters - `app_name, app_logo`.  

app_id would have a prefix of the org_name joined with the app_id requested by a `>`, so for eg an app i wanna create inside tos named briefcase would be `tos>briefcase`. 

Webhook Scope is the scope for the webhook, what updates should be sent via the webhook. It's not the scope of the login.

App creation can happen from the frontend by org_owner/org_admin of an organisation.
...
App_scope is the scope of an application, this defines what all information app gets access to. App_scope would include:
...
If any app decides to add an critical scope in the list of scopes they need, be it IAM or any other application's (obo) critical scope, the applications needs to approve the application. Once the application has been approved for a set of critical scopes, they should be able to remove the scope without permission but for adding any other scope they would again need to request for the new set of scope from the application accordingly. For each application with OBO they would have an initial request defined this would be displayed at the request approval screen as the first message, then the one who is trying to request would be able to write a message. So approval would be like a forum in which the approver could write a message. This is gonna be treated as a normal text message so any of the requestee and approver can write anything in that forum.  

For each application there would be all these verification request which can be viewed by the org_admins and org_owner in the single place, where they can approve the request or deny it, when denying they need to give the reason for denial. Each verification request would be app specific, and a specific app_id gets the approval to use those critical scope endpoints. 

Until this verification is not approved the app is not usable. For say an upgraded verification request clearly show for using this version of upgraded scope you need to verified first, the application with the prior scope is still working. 

Once the verification request has been submited you should also send a mail and for each new reply as well ensure to send the email to the apps as needed, say when the requester sent a new message or an initial verification request is when the verifier app must get an email `app_id` is trying to get approval for critical scopes. And below that the message. And in the email itself the option to reply which would take them to the website. 

And when the verifier rejects/approves/sends a new message the requestee must get an email. Even when the initial verification request is sent send them an acknowledgment email. 
...
For each application login, during the login in IAM, the user can select which all organisations it wanna give the app access to after seeing the application validation screen. Then the consent screen is shown in this screen display all the non critical and critical IAM and external app permissions this app demands, once the user continues from this screen, The user can select all the organisations or at minimum select one organisation. Then the app receives only its declared, user-approved scopes, within the user-selected organizations The app can always do an additive organisation where they can get access to more organisations on top of the already existing orgs. So they can get access to more organisations without needing to loose access to the already existing organisations, and in this as well the same flow would be followed. 

The subject token was issued **to the calling app**, remains valid, and represents the same user throughout the exchange.

For the app they can't trigger organisation specific login, they can just trigger a no scoped login, the user decides the scope of the login by choosing the list of orgs it would like the user to login via. 
...
It should be configurable if i just wanna give someone no consent screen or/and bundle applications. This would also be configured in the backend itself. 
...
For all the apps, it should also be possible for the inter app communications - this can be performed between all the apps in the system. For each such application when an OBO request is initiated check if the application has the particular request in it's app scope and when the user authenticated it was approved by the user. Only when both of these are true OBO should be doable. 
...
It should be possible for any application to fetch the list of exposed API's for OBO for any application and what the metadata requires. 

Each app should be able to configure their list of OBO exposed endpoints and what all do they need in the metadata (if anything). The details for OBO can only be configured by an org_admin or org_owner. 
...
For a given app in a test enviorment i should be able to either:
1) Create an app: Inside an org i should be able to create an app, but for the given app_id it must not already exist in prod. 
2) For the apps that exist in prod they can be imported (importing is restricted to the testing enviorment), you don't even need a organisation for it first, so when i import say `google>drive`, it would create google as an org (with the creator as the owner) and then import the drive application in it (this would have the same app_id, base_url, webhook_url, OBO). For the webhook_secret it would remain the same as defined in the main application hence the webhook_secret is never revealed in the test enviorment. If i configure a new webhook_url, it would return a new secret this can be used. 

For importing say i cant import google>drive inside anyother org except google, as it's owned by google. 

For the test webhook itself also include the testing_key, this would ensure that the test webhook knows which test enviorment does this change belong to. For when a request is sent from a test, instead of including the data, metadata in the webhook directly. We will send it inside a special test object, so if in prod it is: `data: {}, metadata:{}`, for test it would be `test:{data: {}, metadata: {}}`.  

Even the test apps could request the base_url based on the app_id that claims to have sent the request for base_url along with the app_secret we can figure out if it's test, and if test which test layer and show the base url accordingly.


# Testing Layer for other apps

IAM is also gonna manage the testing layer for all the other applications on IAM. What this means is that applications are by default expected that they support test mode.  For test mode how we distinguish it is when the app_secret is being sent along the request the application is in test mode. `Docs note: ensure to make this clear that for your testing enviorments when you recieve the app_secret in the request itself is when the app is in test mode`. 

As IAM we will be managing the test enviorment for every single one application. At a single point of time there can be multiple test enviorments of each application, for each testing enviorment they would be needing the IAM test enviorment that's using this test enviorment. So based on the IAM's test enviorment the application is imported in the IAM test enviorment an app secret is generated as it's a new application in the IAM test enviorment. 

Now say that application has in scope an external application we would also need to introduce that application in the system and as OBO is entirely handled by IAM itself for the application it doesen't need anything. This can trigger apps till any depth and there's no limit by this i mean app a triggers app b as an dependency app b triggers app c and d as dependency and so on and on. 

All the test enviorments must always rely on the IAM test enviorment session currently in place, and during creation they can select if it's in an already existing test enviorment or would this be in a new test enviorment. 

Expose endpoints so each application can trigger internally to create a test enviorment for themselves, so it's possible for the apps as well to expose test enviorment creation on their enviorment itself which would take in name, description (optional), and iam_test_key (optional) if iam test key is given and exists use that enviorment, if given but not exists return error if not given then create a new test enviorment for it. 

Each application can also request all the test enviorments currently active for them. 

Test enviorments are also organisation scoped. So when an app requests for all the test enviorments it would return the test enviorments for that particular organisation. 

Store the internal linking of it all in your db.


Auto deleting of test enviorments for each application: 

By default auto delete the test env of the application after 30 days of no new activity. 
This can be overwritten by application in their app configurations. 


So for creating an test enviorment in any application i should be able to do it one click for any application no matter how tightly dependency based it is. 
...
For the docs keep it as detailed and mention all the details, this is the only thing the other apps can use as their source of knowledge and how they can ensure the login/signup and everything works as intented. 
