...
('00000000-0000-0000-0000-000000000023','other_org','test_admin','Recipient Organization');
...
('other_org>target','application','active',now());
...
SELECT app, scope FROM unnest(ARRAY['test_org>app-alpha','other_org>target']::text[]) app
...
SELECT app, scope, 'test_carbon'::text
FROM unnest(ARRAY['test_org>app-alpha','other_org>target']::text[]) app
CROSS JOIN unnest(ARRAY['self.identity.read','self.membership.read','self.tags.read','directory.tags.read','obo:other_org>target:files.read']) scope;
...
SELECT set_config('iam.principal_id','test_carbon',true),set_config('iam.application_id','test_org>app-alpha',true),set_config('iam.organization_id','00000000-0000-0000-0000-000000000021',true);
...
VALUES('00000000-0000-0000-0000-000000000123',decode(repeat('39',32),'hex'),1,'obo_ijklmnop','test_org>app-alpha','other_org>target','test_carbon','carbon','00000000-0000-0000-0000-000000000021','00000000-0000-0000-0000-000000000031','00000000-0000-0000-0000-000000000101','files.read','{}',1,'POST','/files',decode(repeat('00',32),'hex'),now(),1,1,1,1,now()+interval '60 seconds');
