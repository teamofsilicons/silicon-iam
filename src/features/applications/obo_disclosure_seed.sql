...
('00000000-0000-0000-0000-000000000023','other_org','c:test_admin','Recipient Organization');
...
UPDATE iam.applications SET app_scope='{"iam":["self.identity.read","self.membership.read","self.tags.read","directory.tags.read"],"external":[{"app_id":"target","endpoint_id":"files.read"}]}'
WHERE id='app-alpha';
...
SELECT app, scope FROM unnest(ARRAY['app-alpha','target']::text[]) app
CROSS JOIN unnest(ARRAY['self.identity.read','self.membership.read','self.tags.read','directory.tags.read','obo:target:files.read']) scope;
INSERT INTO iam.application_approved_scopes(application_id,scope,approved_by_carbon_id)
SELECT app, scope, 'c:test_carbon'::text
FROM unnest(ARRAY['app-alpha','target']::text[]) app
CROSS JOIN unnest(ARRAY['self.identity.read','self.membership.read','self.tags.read','directory.tags.read','obo:target:files.read']) scope;
INSERT INTO iam.access_token_scopes(access_token_id,scope)
SELECT '00000000-0000-0000-0000-000000000101',scope
FROM unnest(ARRAY['self.identity.read','self.membership.read','self.tags.read','directory.tags.read','obo:target:files.read']) scope;
...
VALUES('00000000-0000-0000-0000-000000000123',decode(repeat('39',32),'hex'),1,'obo_ijklmnop','app-alpha','target','c:test_carbon','carbon','00000000-0000-0000-0000-000000000021','00000000-0000-0000-0000-000000000031','00000000-0000-0000-0000-000000000101','files.read','{}',1,'POST','/files',decode(repeat('00',32),'hex'),now(),1,1,1,1,now()+interval '60 seconds');
