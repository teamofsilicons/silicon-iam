...
use super::{
    model::{
        ...
        SiliconTokenRotatedResponse,
    },
    support::{self, Claim, MutationEvent},
    validation,
};

const ROLE_REQUEST_ROUTE: &str = "/api/v1/organizations/{org_id}/role-change-requests";
...
const ROTATION_COMPLETE_ROUTE: &str = "/api/v1/organizations/{org_id}/silicons/{silicon_id}/token-rotation-requests/{request_id}/complete";
...
struct ApprovalRow {
    ...
    previous_credential_id: Option<Uuid>,
...
pub(super) async fn create_role_change_request(
    ...
    support::require_capability(&scope.access, Capability::RolesRequest)?;
    ...
    sqlx::query(
        ...
        INSERT INTO iam.approval_requests (
            ...
            minimum_distinct_approvers, expires_at
        ) VALUES ($1, $2, $3, $4, $5, transaction_timestamp() + interval '48 hours')
        ",
    )
    .bind(request_id)
...
pub(super) async fn decide_approval_request(
    ...
    if approval.status != "pending" || approval.expires_at <= OffsetDateTime::now_utc() {
    ...
    support::record_mutation(
        ...
        MutationEvent {
            ...
            metadata: json!({ "approval_request_id": request_id, "decision": input.decision }),
        },
    )
    .await?;
    let body = support::finish_json(
...
pub(super) async fn request_silicon_token_rotation(
    ...
    let lease = match support::claim(
        ...
        &json!({ "silicon_id": silicon_handle }),
        false,
    )
    .await?
    {
    ...
    support::record_mutation(
        ...
        MutationEvent {
            ...
            before_state: None,
            after_state: Some(approval.immutable_payload.clone()),
            metadata: json!({ "approval_request_id": request_id, "silicon_id": target.silicon_id }),
        },
    )
    .await?;
    let body = support::finish_json(
...
pub(super) async fn complete_silicon_token_rotation(
    ...
    let lease = match support::claim(
        ...
        &json!({ "silicon_id": silicon_handle, "request_id": request_id }),
        true,
    )
    .await?
    {
    ...
    let ready = sqlx::query_scalar::<_, bool>(
        ...
        SELECT EXISTS (
            ...
            WHERE rotation.organization_id = $1 AND rotation.approval_request_id = $2
              ...
              AND request.status = 'approved' AND request.expires_at > transaction_timestamp()
        )
        ",
    )
    .bind(scope.access.organization_id)
    ...
    support::record_mutation(
        ...
        MutationEvent {
            ...
            aggregate_type: "silicon_credential",
            aggregate_id: replacement_id,
            aggregate_version: credential_version,
            event_type: "organization.silicon.credential_rotated.v1",
            before_state: None,
            after_state: Some(json!({ "credential_version": credential_version })),
            metadata: json!({ "silicon_id": target.silicon_id, "approval_request_id": request_id }),
        },
    )
    .await?;
    let body = support::finish_json(
            ...
            "current_owner_or_admin" => {
                authority.org_role == OrgRole::Owner
                    || (authority.org_role == OrgRole::Admin
                        && requirement.required_capability.as_deref() == Some("roles.approve")
                        && authority.allows(Capability::RolesApprove))
            }
            _ => false,
        };
        if eligible {
...
async fn materialize_approval(
    ...
    let immutable_payload = match row.kind.as_str() {
        ...
        "silicon_token_rotation" => json!({
            ...
            "previous_credential_id": row.previous_credential_id,
        }),
        _ => json!({}),
    };
...
async fn revoke_silicon_sessions(
    ...
    sqlx::query(
        "UPDATE iam.authentication_sessions SET status = 'revoked', revoked_at = transaction_timestamp(), revocation_reason = 'Silicon credential rotated' WHERE subject_principal_id = $1 AND status = 'active'",
    )
    .bind(silicon_id)
    .execute(&mut **transaction)
    .await
...
fn validate_approval_filters(query: &ApprovalQuery) -> Result<(), AppError> {
    ...
    if query.kind.as_deref().is_some_and(|value| {
        !matches!(
            value,
            "carbon_job_role_change" | "silicon_job_role_change" | "silicon_token_rotation"
        )
    }) {
...
const APPROVAL_LIST_SQL: &str = r"
    SELECT request.id, organization.org_id, request.request_kind AS kind,
           CASE WHEN request.status = 'pending' AND request.expires_at <= transaction_timestamp() THEN 'expired' ELSE request.status END AS status,
           ...
           COALESCE(role_change.target_membership_id, rotation_silicon.membership_id) AS target_membership_id,
           role_change.previous_job_role, role_change.proposed_job_role, rotation.silicon_id, rotation.previous_credential_id,
           request.expires_at, COALESCE(request.applied_at, request.rejected_at, request.cancelled_at) AS completed_at,
    ...
    WHERE request.organization_id = $1 AND ($2::uuid IS NULL OR request.id > $2)
      AND ($4::text IS NULL OR CASE WHEN request.status = 'pending' AND request.expires_at <= transaction_timestamp() THEN 'expired' WHEN request.status = 'applied' THEN 'completed' ELSE request.status END = $4)
