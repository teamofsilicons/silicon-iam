...
BEGIN;
SELECT set_config('iam.principal_id','00000000-0000-0000-0000-000000000002',true),
...
END $$;
INSERT INTO iam.application_requested_scopes(application_id,scope) SELECT '00000000-0000-0000-0000-000000000011',scope FROM unnest(ARRAY['directory.carbons.read','directory.profiles.read','directory.silicons.read','organization.invitations.read']) scope;
INSERT INTO iam.application_approved_scopes(application_id,scope,approved_by_carbon_id) SELECT '00000000-0000-0000-0000-000000000011',scope,'00000000-0000-0000-0000-000000000001' FROM unnest(ARRAY['directory.carbons.read','directory.profiles.read','directory.silicons.read','organization.invitations.read']) scope;
...
DO $$ DECLARE scopes text[]; BEGIN
 ...
 SELECT array_agg(scope ORDER BY scope) INTO scopes FROM iam_private.list_profile_webhook_authorization_scopes('00000000-0000-0000-0000-000000000002');
 ...
 IF NOT EXISTS(SELECT 1 FROM iam_private.current_application_resource_scopes('00000000-0000-0000-0000-000000000011','00000000-0000-0000-0000-000000000002','00000000-0000-0000-0000-000000000021') scope WHERE scope='directory.profiles.read') THEN
...
END $$;
RESET ROLE;
UPDATE iam.application_approved_scopes SET revoked_at=transaction_timestamp(),revoked_by_carbon_id='00000000-0000-0000-0000-000000000001' WHERE application_id='00000000-0000-0000-0000-000000000011' AND scope='directory.profiles.read';
SET LOCAL ROLE silicon_iam_api;
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM iam_private.current_application_resource_scopes('00000000-0000-0000-0000-000000000011','00000000-0000-0000-0000-000000000002','00000000-0000-0000-0000-000000000021') scope WHERE scope='directory.profiles.read') THEN
