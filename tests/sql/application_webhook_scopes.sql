...
BEGIN;
SELECT set_config('iam.principal_id','test_admin',true),
...
END $$;
INSERT INTO iam.application_requested_scopes(application_id,scope) SELECT 'test_org>app-alpha',scope FROM unnest(ARRAY['directory.carbons.read','directory.profiles.read','directory.silicons.read','organization.invitations.read']) scope;
INSERT INTO iam.application_approved_scopes(application_id,scope,approved_by_carbon_id) SELECT 'test_org>app-alpha',scope,'test_carbon' FROM unnest(ARRAY['directory.carbons.read','directory.profiles.read','directory.silicons.read','organization.invitations.read']) scope;
...
DO $$ DECLARE scopes text[]; BEGIN
 ...
 SELECT array_agg(scope ORDER BY scope) INTO scopes FROM iam_private.list_profile_webhook_authorization_scopes('test_admin');
 ...
 IF NOT EXISTS(SELECT 1 FROM iam_private.current_application_resource_scopes('test_org>app-alpha','test_admin','00000000-0000-0000-0000-000000000021') scope WHERE scope='directory.profiles.read') THEN
...
END $$;
RESET ROLE;
UPDATE iam.application_approved_scopes SET revoked_at=transaction_timestamp(),revoked_by_carbon_id='test_carbon' WHERE application_id='test_org>app-alpha' AND scope='directory.profiles.read';
SET LOCAL ROLE silicon_iam_api;
DO $$ BEGIN
 IF EXISTS(SELECT 1 FROM iam_private.current_application_resource_scopes('test_org>app-alpha','test_admin','00000000-0000-0000-0000-000000000021') scope WHERE scope='directory.profiles.read') THEN
