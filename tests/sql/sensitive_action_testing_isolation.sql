...
INSERT INTO iam.principals(id,kind,status,activated_at) VALUES
 ('isolation-owner','carbon','active',now()),('chef:isolation-test','silicon','active',now());
INSERT INTO iam.carbons(id,carbon_id,display_name) VALUES('isolation-owner','isolation-owner','First Owner');
INSERT INTO iam.organizations(id,org_id,name,created_by_carbon_id) VALUES
 ('01120000-0000-0000-0000-000000000101','isolation-test','First Org','isolation-owner');
INSERT INTO iam.organization_memberships(id,organization_id,principal_id,principal_kind,org_role) VALUES
 ('01120000-0000-0000-0000-000000000111','01120000-0000-0000-0000-000000000101','isolation-owner','carbon','owner'),
 ('01120000-0000-0000-0000-000000000112','01120000-0000-0000-0000-000000000101','chef:isolation-test','silicon','member');
INSERT INTO iam.silicons(id,organization_id,membership_id,organization_handle,silicon_handle,display_name) VALUES
 ('chef:isolation-test','01120000-0000-0000-0000-000000000101','01120000-0000-0000-0000-000000000112','isolation-test','chef','First Chef');
SELECT set_config('iam.testing_environment_id','01120000-0000-0000-0000-000000000092',true);
INSERT INTO iam.principals(id,kind,status,activated_at) VALUES
 ('isolation-owner','carbon','active',now()),('chef:isolation-test','silicon','active',now());
INSERT INTO iam.carbons(id,carbon_id,display_name) VALUES('isolation-owner','isolation-owner','Second Owner');
INSERT INTO iam.organizations(id,org_id,name,created_by_carbon_id) VALUES
 ('01120000-0000-0000-0000-000000000201','isolation-test','Second Org','isolation-owner');
INSERT INTO iam.organization_memberships(id,organization_id,principal_id,principal_kind,org_role) VALUES
 ('01120000-0000-0000-0000-000000000211','01120000-0000-0000-0000-000000000201','isolation-owner','carbon','owner'),
 ('01120000-0000-0000-0000-000000000212','01120000-0000-0000-0000-000000000201','chef:isolation-test','silicon','member');
INSERT INTO iam.silicons(id,organization_id,membership_id,organization_handle,silicon_handle,display_name) VALUES
 ('chef:isolation-test','01120000-0000-0000-0000-000000000201','01120000-0000-0000-0000-000000000212','isolation-test','chef','Second Chef');

SELECT set_config('iam.testing_environment_id','01120000-0000-0000-0000-000000000091',true),
       set_config('iam.organization_id','01120000-0000-0000-0000-000000000101',true),
       set_config('iam.principal_id','isolation-owner',true);
...
DO $$ DECLARE v_result jsonb; BEGIN
 ...
 PERFORM set_config('iam.principal_id','chef:isolation-test',true);
 ...
 IF iam_private.lock_silicon_self_profile('01120000-0000-0000-0000-000000000101','chef:isolation-test')<>'01120000-0000-0000-0000-000000000112' THEN
  RAISE EXCEPTION 'self-profile helper crossed environments';
 END IF;
 IF NOT iam_private.update_silicon_self_profile('01120000-0000-0000-0000-000000000101','chef:isolation-test',1,NULL,'Asia/Kolkata',false,NULL,false,NULL) THEN
  RAISE EXCEPTION 'first environment self-profile update failed';
 END IF;
 BEGIN
  PERFORM iam_private.lock_silicon_self_profile('01120000-0000-0000-0000-000000000201','chef:isolation-test');
 ...
 IF NOT EXISTS(SELECT 1 FROM iam.silicons WHERE id='chef:isolation-test' AND timezone_id='UTC' AND version=1) THEN
...
UPDATE iam.principals SET status='suspended',suspended_at=now()
 WHERE id='chef:isolation-test' AND testing_environment_id='01120000-0000-0000-0000-000000000091';
SELECT set_config('iam.testing_environment_id','01120000-0000-0000-0000-000000000091',true),
       set_config('iam.organization_id','01120000-0000-0000-0000-000000000101',true);
SET LOCAL ROLE silicon_iam_api;
DO $$ BEGIN
 ...
 EXCEPTION WHEN insufficient_privilege THEN NULL; END;
 BEGIN
  PERFORM iam_private.lock_silicon_self_profile('01120000-0000-0000-0000-000000000101','chef:isolation-test');
...
DO $$ BEGIN
 ...
 END IF;
 IF NOT EXISTS(SELECT 1 FROM iam.silicons WHERE id='chef:isolation-test'
