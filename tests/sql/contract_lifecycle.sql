...
BEGIN;
UPDATE iam_private.contract_versions
SET last_requested_at = clock_timestamp() - interval '30 days'
WHERE version = 'v1';
INSERT INTO iam_private.contract_versions(version,status,deprecated_at,last_requested_at) VALUES
...
RESET ROLE;
DO $$ BEGIN
 IF (SELECT status FROM iam_private.contract_versions WHERE version='v1') <> 'current' THEN
  RAISE EXCEPTION 'current contract must never sunset from inactivity';
 END IF;
 IF (SELECT status FROM iam_private.contract_versions WHERE version='v991') <> 'deprecated' THEN
  RAISE EXCEPTION 'active deprecated contract must remain available';
 END IF;
 IF (SELECT status FROM iam_private.contract_versions WHERE version='v992') <> 'sunset' THEN
  RAISE EXCEPTION 'deprecated contract idle longer than seven days must sunset';
 END IF;
 IF (SELECT status FROM iam_private.contract_versions WHERE version='v993') <> 'deprecated' THEN
