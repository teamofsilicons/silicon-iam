...
SELECT 'app-alpha',unnest(ARRAY['self.membership.read','roles.read']);
INSERT INTO iam.application_approved_scopes(application_id,scope,approved_by_carbon_id)
SELECT 'app-alpha',unnest(ARRAY['self.membership.read','roles.read']),'c:test_carbon';
...
SELECT set_config('iam.principal_id','c:test_carbon',true),
 set_config('iam.application_id','app-alpha',true),set_config('iam.organization_id','',true);
SET LOCAL ROLE silicon_iam_api;
DO $$ DECLARE listed jsonb; BEGIN
 listed:=iam_private.list_current_application_authorizations('00000000-0000-0000-0000-000000000101','c:test_carbon','app-alpha',1);
...
RESET ROLE;
INSERT INTO iam.access_token_scopes(access_token_id,scope) VALUES('00000000-0000-0000-0000-000000000101','self.membership.read');
UPDATE iam.application_approved_scopes SET revoked_at=transaction_timestamp(),revoked_by_carbon_id='c:test_carbon' WHERE application_id='app-alpha' AND scope='self.membership.read';
SET LOCAL ROLE silicon_iam_api;
DO $$ DECLARE listed jsonb; BEGIN
 listed:=iam_private.list_current_application_authorizations('00000000-0000-0000-0000-000000000101','c:test_carbon','app-alpha',1);
...
INSERT INTO iam.application_requested_scopes(application_id,scope)
SELECT 'app-alpha',unnest(ARRAY['self.tags.read','memberships.read']);
INSERT INTO iam.application_approved_scopes(application_id,scope,approved_by_carbon_id)
SELECT 'app-alpha',unnest(ARRAY['self.tags.read','memberships.read']),'c:test_carbon';
...
RESET ROLE;
INSERT INTO iam.access_token_scopes(access_token_id,scope) VALUES('00000000-0000-0000-0000-000000000101','self.tags.read');
UPDATE iam.application_approved_scopes SET revoked_at=transaction_timestamp(),revoked_by_carbon_id='c:test_carbon' WHERE application_id='app-alpha' AND scope='self.tags.read';
SET LOCAL ROLE silicon_iam_api;
DO $$ DECLARE listed jsonb; BEGIN
 listed:=iam_private.list_current_application_authorizations('00000000-0000-0000-0000-000000000101','c:test_carbon','app-alpha',1);
