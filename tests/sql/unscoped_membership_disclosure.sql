...
INSERT INTO iam.application_requested_scopes(application_id,scope)
SELECT '00000000-0000-0000-0000-000000000011',unnest(ARRAY['self.tags.read','memberships.read']);
INSERT INTO iam.application_approved_scopes(application_id,scope,approved_by_carbon_id)
SELECT '00000000-0000-0000-0000-000000000011',unnest(ARRAY['self.tags.read','memberships.read']),'00000000-0000-0000-0000-000000000001';
...
RESET ROLE;
INSERT INTO iam.access_token_scopes(access_token_id,scope) VALUES('00000000-0000-0000-0000-000000000101','self.tags.read');
UPDATE iam.application_approved_scopes SET revoked_at=transaction_timestamp(),revoked_by_carbon_id='00000000-0000-0000-0000-000000000001' WHERE application_id='00000000-0000-0000-0000-000000000011' AND scope='self.tags.read';
SET LOCAL ROLE silicon_iam_api;
DO $$ DECLARE listed jsonb; BEGIN
 listed:=iam_private.list_current_application_authorizations('00000000-0000-0000-0000-000000000101','00000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000011',1);
