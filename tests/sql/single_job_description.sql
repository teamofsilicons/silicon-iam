...
INSERT INTO iam.principals(id,kind,status,activated_at) VALUES
 ('job-owner','carbon','active',now()),('job-fallback','carbon','active',now()),
 ('job-empty','carbon','active',now()),('helper:job-test','silicon','active',now());
INSERT INTO iam.carbons(id,carbon_id,display_name,description) VALUES
 ('job-owner','job-owner','Owner','Do not replace the existing job'),
 ('job-fallback','job-fallback','Fallback',E'  Fallback carbon description\n'),
 ('job-empty','job-empty','Empty',NULL);
INSERT INTO iam.organizations(id,org_id,created_by_carbon_id,name) VALUES
 ('01130000-0000-0000-0000-000000000010','job-test','job-owner','Job Test');
INSERT INTO iam.organization_memberships(id,organization_id,principal_id,principal_kind,org_role,job_role) VALUES
 ('01130000-0000-0000-0000-000000000011','01130000-0000-0000-0000-000000000010','job-owner','carbon','owner','  Existing job text  '),
 ('01130000-0000-0000-0000-000000000012','01130000-0000-0000-0000-000000000010','job-fallback','carbon','member',''),
 ('01130000-0000-0000-0000-000000000013','01130000-0000-0000-0000-000000000010','job-empty','carbon','member',''),
 ('01130000-0000-0000-0000-000000000014','01130000-0000-0000-0000-000000000010','helper:job-test','silicon','member','');
INSERT INTO iam.silicons(id,organization_id,membership_id,organization_handle,silicon_handle,display_name,description) VALUES
 ('helper:job-test','01130000-0000-0000-0000-000000000010','01130000-0000-0000-0000-000000000014','job-test','helper','Helper','Fallback silicon description');
...
WHERE carbon.id IN ('job-owner','job-fallback','job-empty');
...
DO $$ BEGIN
 IF (SELECT job_role FROM iam.organization_memberships WHERE principal_id='job-owner') IS DISTINCT FROM '  Existing job text  ' THEN
  RAISE EXCEPTION 'Existing nonempty job description was modified'; END IF;
 IF (SELECT job_role FROM iam.organization_memberships WHERE principal_id='job-fallback') IS DISTINCT FROM E'  Fallback carbon description\n' THEN
  RAISE EXCEPTION 'Carbon description fallback missing'; END IF;
 IF (SELECT job_role FROM iam.organization_memberships WHERE principal_id='helper:job-test') IS DISTINCT FROM 'Fallback silicon description' THEN
  RAISE EXCEPTION 'Silicon description fallback missing'; END IF;
 IF (SELECT job_role FROM iam.organization_memberships WHERE principal_id='job-empty') IS DISTINCT FROM '' THEN
