...
INSERT INTO iam.principals (id, kind, status, activated_at) VALUES
  ('00000000-0000-0000-0000-000000000801', 'carbon', 'active', transaction_timestamp());
INSERT INTO iam.carbons (id, carbon_id, display_name) VALUES
  ('00000000-0000-0000-0000-000000000801', 'new_account', 'New Account');
INSERT INTO iam.authentication_sessions (
  ...
  '00000000-0000-0000-0000-000000000801', 'carbon', 'email_otp', 1, 1,
...
INSERT INTO iam.application_requested_scopes (application_id, scope) VALUES
  ('00000000-0000-0000-0000-000000000011', 'organizations.create');
INSERT INTO iam.application_approved_scopes (application_id, scope, approved_by_carbon_id) VALUES
  ('00000000-0000-0000-0000-000000000011', 'organizations.create', '00000000-0000-0000-0000-000000000001');
SELECT set_config('iam.principal_id','00000000-0000-0000-0000-000000000801',true),
...
DO $$ DECLARE selected uuid[]; BEGIN
  selected := iam_private.lock_account_login_organization_selection(
    '00000000-0000-0000-0000-000000000801','00000000-0000-0000-0000-000000000841',
    '{}'::text[], '00000000-0000-0000-0000-000000000011');
  ...
  BEGIN
    PERFORM iam_private.lock_account_login_organization_selection(
      '00000000-0000-0000-0000-000000000801','00000000-0000-0000-0000-000000000841',
      '{}'::text[], '00000000-0000-0000-0000-000000000012');
  ...
  BEGIN
    PERFORM iam_private.lock_account_login_organization_selection(
      '00000000-0000-0000-0000-000000000801','00000000-0000-0000-0000-000000000041',
      '{}'::text[], '00000000-0000-0000-0000-000000000011');
  ...
  BEGIN
    PERFORM iam_private.lock_account_login_organization_selection(
      '00000000-0000-0000-0000-000000000801','00000000-0000-0000-0000-000000000841',
      NULL::text[], '00000000-0000-0000-0000-000000000011');
...
UPDATE iam.application_approved_scopes SET revoked_at=transaction_timestamp(),
  revoked_by_carbon_id='00000000-0000-0000-0000-000000000001'
WHERE application_id='00000000-0000-0000-0000-000000000011' AND scope='organizations.create';
...
RESET ROLE;
INSERT INTO iam.application_requested_scopes (application_id, scope) VALUES
  ('00000000-0000-0000-0000-000000000011', 'organizations.join');
INSERT INTO iam.application_approved_scopes (application_id, scope, approved_by_carbon_id) VALUES
  ('00000000-0000-0000-0000-000000000011', 'organizations.join', '00000000-0000-0000-0000-000000000001');
SET LOCAL ROLE silicon_iam_api;
DO $$ BEGIN
  IF iam_private.lock_account_login_organization_selection(
    '00000000-0000-0000-0000-000000000801','00000000-0000-0000-0000-000000000841',
    '{}'::text[], '00000000-0000-0000-0000-000000000011') IS DISTINCT FROM '{}'::uuid[] THEN
...
INSERT INTO iam.oauth_consent_grants (
  ...
  '00000000-0000-0000-0000-000000000871', '00000000-0000-0000-0000-000000000011',
  '00000000-0000-0000-0000-000000000801', 'carbon',
  '00000000-0000-0000-0000-000000000841', '{}'
);
...
INSERT INTO iam.access_tokens (
  ...
  '00000000-0000-0000-0000-000000000841', '00000000-0000-0000-0000-000000000801', 'carbon',
  '00000000-0000-0000-0000-000000000011', 'test_org>app-alpha', '00000000-0000-0000-0000-000000000011',
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM iam_private.lock_current_application_oauth_subject_authority(
    '00000000-0000-0000-0000-000000000011','00000000-0000-0000-0000-000000000871',
    '00000000-0000-0000-0000-000000000841','00000000-0000-0000-0000-000000000801',
...
DO $$ BEGIN
  IF iam_private.list_current_application_authorizations(
    '00000000-0000-0000-0000-000000000881','00000000-0000-0000-0000-000000000801',
    '00000000-0000-0000-0000-000000000011',1) IS DISTINCT FROM '[]'::jsonb THEN
...
VALUES ('00000000-0000-0000-0000-000000000831','00000000-0000-0000-0000-000000000021',
  '00000000-0000-0000-0000-000000000801','carbon','member');
SET LOCAL ROLE silicon_iam_api;
DO $$ BEGIN
