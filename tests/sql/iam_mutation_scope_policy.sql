...
DO $$
BEGIN
 IF iam_private.application_iam_scope_allowed('app-alpha','organization.invitations.create') THEN
   RAISE EXCEPTION 'untrusted application acquired restricted permission';
 END IF;
 BEGIN
   PERFORM iam_private.configure_application_scopes('app-alpha',
 ...
 PERFORM iam_private.configure_application_scopes('app-alpha',
   ...
   'c:test_carbon');
 IF EXISTS (SELECT 1 FROM iam.application_approved_scopes WHERE application_id='app-alpha' AND scope='organizations.create' AND revoked_at IS NULL) THEN
...
SET LOCAL ROLE silicon_iam_api;
SELECT iam_private.configure_application_scopes('app-alpha',
...
INSERT INTO iam.application_approved_scopes(application_id,scope,approved_by_carbon_id) VALUES
 ('app-alpha','organization.invitations.create','c:test_carbon');
...
DO $$
BEGIN
 IF EXISTS(SELECT 1 FROM iam.application_approved_scopes WHERE application_id='app-alpha' AND scope='organization.invitations.create' AND revoked_at IS NULL) THEN
 ...
 END IF;
 IF NOT iam_private.application_iam_scope_allowed('app-alpha','organization.silicons.create') THEN
...
DO $$ BEGIN
 IF iam_private.application_iam_scope_allowed('app-alpha','organization.silicons.create') THEN
   RAISE EXCEPTION 'removing trust retained restricted permission';
 END IF;
 IF NOT iam_private.application_iam_scope_allowed('app-alpha','organizations.create') THEN
...
UPDATE iam.organizations SET trusted_org=true,
 ...
 WHERE id='00000000-0000-0000-0000-000000000021';
DO $$ BEGIN
 IF NOT iam_private.application_iam_scope_allowed('app-alpha','organization.invitations.create') THEN
 ...
 IF NOT EXISTS (SELECT 1 FROM iam.application_approved_scopes
   WHERE application_id='app-alpha'
