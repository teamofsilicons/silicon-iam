-- A delegated endpoint may receive only explicitly consented self disclosures.
-- All parent/proof/epoch/membership/endpoint locks and checks remain unchanged.
-- Ordinary bearer projection is unchanged; proof identity remains available in
-- the outer verified actor binding independently of optional snapshot fields.
...
REVOKE ALL ON FUNCTION iam_private.get_current_application_authorization(uuid, uuid, uuid, uuid, uuid, bigint, uuid) FROM PUBLIC;
