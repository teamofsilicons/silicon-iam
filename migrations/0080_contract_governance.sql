...
CREATE TABLE iam.contract_versions (
...
INSERT INTO iam.contract_versions(version,status) VALUES('v1','current');
...
BEGIN
 UPDATE iam.contract_versions SET last_requested_at=clock_timestamp()
 ...
 SELECT status INTO current_status FROM iam.contract_versions WHERE version=p_version FOR SHARE;
...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, iam, iam_private AS $$
 ...
 'policy',jsonb_build_object('initial_version','v1','breaking_changes','new_major_version','sunset_after_idle_days',7)) FROM iam.contract_versions
$$;
...
BEGIN
 ...
 UPDATE iam.contract_versions SET status='sunset',sunset_at=clock_timestamp()
